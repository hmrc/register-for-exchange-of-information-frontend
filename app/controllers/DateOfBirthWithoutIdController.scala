...
class DateOfBirthWithoutIdController @Inject() (
  ...
  private def render(mode: Mode, form: Form[LocalDate])(implicit request: DataRequest[AnyContent]): Future[Html] = {
    val data = Json.obj(
      "form"   -> form,
      "action" -> routes.DateOfBirthWithoutIdController.onSubmit(mode).url,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] =
    standardActionSets.identifiedUserWithData().async {
      implicit request =>
        render(mode, request.userAnswers.get(DateOfBirthWithoutIdPage).fold(form)(form.fill)).map(Ok(_))
    }
