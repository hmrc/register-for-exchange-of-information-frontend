...
import models.{Mode, NonUkName, Regime}
...
class NonUkNameController @Inject() (
  ...
  def onPageLoad(mode: Mode, regime: Regime): Action[AnyContent] =
    standardActionSets.identifiedUserWithData(regime).async {
      implicit request =>
        render(mode, regime, request.userAnswers.get(NonUkNamePage).fold(form)(form.fill)).map(Ok(_))
    }
      ...
      implicit request =>
        form
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.setOrCleanup(NonUkNamePage, value, checkPreviousUserAnswer = true))
