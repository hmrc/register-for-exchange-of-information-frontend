...
object DateInput extends NunjucksSupport {
  ...
  def localDate(field: Field): ViewModel = {
    ...
    val error: Option[Text.Message] = (field.error orElse field("day").error orElse field("month").error orElse field("year").error)
      .map(
    ...
    def classes(fieldName: String, classes: String*): String =
      error match {
        ...
        case Some(err)
            if !err.args.map(_.toString).contains("day") &
              !err.args.map(_.toString).contains("month") &
              !err.args.map(_.toString).contains("year") &
              err.args.nonEmpty =>
          (" govuk-input--error" :: classes.toList).mkString(" ")
        case Some(_) =>
          classes.toList.mkString(" ")
