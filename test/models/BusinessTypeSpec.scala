...
class BusinessTypeSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {
  ...
  "BussinessType" - {
    ...
    "must deserialise valid values" in {
      ...
      forAll(gen) {
        bussinessType =>
          JsString(bussinessType.toString).validate[BusinessType].asOpt.value mustEqual bussinessType
      }
    }
    ...
    "must serialise" in {
      ...
      forAll(gen) {
        bussinessType =>
          Json.toJson(bussinessType) mustEqual JsString(bussinessType.toString)
      }
    }
  }
}
