    ...
    "in CheckMode mode" - {
      ...
      "must go from Reporter type page" - {
        ...
        "to Registered address in UK page if any organisation type or sole trader is selected" in {
          forAll(arbitrary[UserAnswers]) { answers =>
            val orgOrSoleTraderReporters =
              List(LimitedCompany, LimitedPartnership, Partnership, UnincorporatedAssociation, Sole)

            orgOrSoleTraderReporters.foreach { organisation =>
              val updatedAnswers =
                answers
      ...
      "must go from Do you have UTR page" - {
        ...
        "to business without ID name page if No is selected and reporter type is any organisation" in {
          forAll(arbitrary[UserAnswers]) { answers =>
            ...
            organisationReporters.foreach { organisation =>
              val updatedAnswers =
                answers
                  .set(ReporterTypePage, organisation)
                  .success
      ...
      "must go from 'What is your Unique Tax Payer Reference' (utr) page to " - {
        ...
        "'What is your Name' page if the business type is 'Sole trader'" in {
          ...
          forAll(arbitrary[UserAnswers], arbitraryUniqueTaxpayerReference.arbitrary) { (answers, utr) =>
            val updatedAnswers = answers
              .set(ReporterTypePage, Sole)
              .success
              .value
        ...
        val updatedAnswers =
          answers
            .set(
              ...
              Address("Jarrow", None, "Park", None, None, Country("", "GB", "United Kingdom"))
            )
            .success
            .value
