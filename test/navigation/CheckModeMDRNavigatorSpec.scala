...
class CheckModeMDRNavigatorSpec extends SpecBase with ScalaCheckPropertyChecks with Generators {
  ...
  "Navigator" - {
    ...
    "in CheckMode mode" - {
      ...
      "must go from 'Do You Have Unique Tax Payer Reference?' page to 'CheckYourAnswers' page " +
        "if user has not changed answer & contact details still exist" in {
      ...
      "must go from 'What Are You Registering As' page to 'Check Your Answers' page" +
        "if user has not changed answer & contact details still exist" in {
